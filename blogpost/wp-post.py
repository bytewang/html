...
class TCParser():
    ...
    def json2wp(self):
        if (len(self.pagelist)<1):
            return False;
        i=0;
        title = json.dumps(self.pagelist[0]["title"],ensure_ascii=False).encode('UTF-8').strip('"');
        body = "<div id='content'>";
        while i<len(self.pagelist):
            page = self.pagelist[i];
            content = json.dumps(page["content"],ensure_ascii=False).encode('UTF-8').strip('"');
            ...
            i+=1;
        body += "</div>\n";
        os.system("rm wp-data/* -fr");
        file = open("wp-data/wp-data.html","w");
        file.write(body);
        command = "./blogpost.py  -t \'" + title + "\' post " + "wp-data/wp-data.html";
        os.system(command);
